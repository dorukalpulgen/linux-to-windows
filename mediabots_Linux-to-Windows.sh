...
link1_status=$(curl -Is http://139.178.73.5/WS2012R2.ISO | grep HTTP | cut -f2 -d" ")
link2_status=$(curl -Is http://139.178.73.5/WS2012R2.ISO | grep HTTP | cut -f2 -d" ")
#sudo wget -P /mediabots https://archive.org/download/WS2012R2/WS2012R2.ISO # Windows Server 2012 R2 
if [ $link1_status = "200" ] ; then 
	sudo wget -P /mediabots http://139.178.73.5/WS2012R2.ISO
